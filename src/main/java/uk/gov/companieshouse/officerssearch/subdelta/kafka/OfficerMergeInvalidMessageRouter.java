...
public class OfficerMergeInvalidMessageRouter implements ProducerInterceptor<String, OfficerMergeData> {
    ...
    @Override
    public ProducerRecord<String, OfficerMergeData> onSend(
            ProducerRecord<String, OfficerMergeData> producerRecord) {
        if (messageFlags.isRetryable()) {
            ...
            String exception = Optional.ofNullable(
                    ...
                    .orElse("unknown");
            OfficerMergeData invalidData = new OfficerMergeData("", "", "");

            final String msg = "{ \"invalid_message\": \"exception: [ %s ] redirecting message from topic: %s, partition: %d, offset: %d to invalid topic\" }"
                    .formatted(exception, originalTopic, partition, offset);

            LOGGER.info("Moving record into topic: [%s]%nMessage content: %s"
                    .formatted(invalidMessageTopic, msg), DataMapHolder.getLogMap());
