...
module "ecs-service" {
  ...
  container_version = var.officers_search_consumer_version
...
module "ecs-service-old-kafka" {
  ...
  docker_registry   = var.docker_registry
  docker_repo       = local.docker_repo
  container_version = var.officers_search_consumer_old_kafka_version
  container_port    = local.container_port
...
module "secrets" {
  source = "git@github.com:companieshouse/terraform-modules//aws/ecs/secrets?ref=1.0.427"
